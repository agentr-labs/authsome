...
def test_discover_registration_endpoint_success():
    ...
    with patch(
        "authsome.flows.dcr_pkce.http_client.get", side_effect=[mock_resp1, mock_resp2]
    ):
...
def test_discover_registration_endpoint_failure():
    ...
    with patch(
        ...
        with pytest.raises(
            DiscoveryError, match="Could not discover registration_endpoint"
        ):
            flow._discover_registration_endpoint(provider)
...
def test_dcr_pkce_flow_success(tmp_path):
    ...
    with patch(
        "authsome.flows.dcr_pkce.http_client.post", side_effect=[dcr_resp, token_resp]
    ):
        with patch("authsome.flows.dcr_pkce.webbrowser.open", side_effect=mock_open):
            with patch(
                ...
                record = flow.authenticate(
                    provider, crypto, "default", "default", scopes=["test"]
                )
...
def test_dcr_pkce_flow_reuse_client(tmp_path):
    ...
    with patch("authsome.flows.dcr_pkce.http_client.post", return_value=token_resp):
        with patch("authsome.flows.dcr_pkce.webbrowser.open", side_effect=mock_open):
            ...
            record = flow.authenticate(
                provider, crypto, "default", "default", client_id="existing_cid"
            )
...
def test_exchange_code_http_error(tmp_path):
    ...
    with patch("authsome.flows.dcr_pkce.webbrowser.open", side_effect=mock_open):
        with patch(
            ...
            with pytest.raises(
                AuthenticationFailedError, match="Token exchange failed"
            ):
                flow.authenticate(
                    provider, crypto, "default", "default", client_id="cid"
                )
